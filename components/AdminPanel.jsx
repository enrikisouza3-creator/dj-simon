...
async function supaAuthCreate(email, password, serviceKey) {
  ...
  if (!res.ok) {
    // A Admin API do GoTrue/Supabase retorna o erro em campos diferentes
    // conforme a versão: msg, error_description, error, ou message.
    // Usar só "data.message" faz cair sempre no fallback genérico.
    const realMsg =
      data.msg || data.error_description || data.error || data.message ||
      `HTTP ${res.status} sem corpo de erro legível`;
